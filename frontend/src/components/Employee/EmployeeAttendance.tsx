...
export default function EmployeeAttendance({
  ...
  const calculateWorkingHours = () => {
    if (!todayRecord?.inTime)
      return { canCheckout: true, hoursWorked: 0, minutesRemaining: 0 };
  ...
  const loadToday = useCallback(async () => {
    if (!userId) return;
    try {
      const response = await attendanceAPI.getAttendance(userId, {
        date: today,
      });
  ...
  const calculateWorkingHoursFromTimes = (
    inTime?: string,
    outTime?: string
  ): string => {
  ...
  async function handleCheckIn() {
    ...
    try {
      ...
      // directly use data
      const { data: result } = await attendanceAPI.checkIn(payload);
    ...
    try {
      ...
      if (!isAutoCheckout) {
        toast({
          ...
          description: `You need to work for 9 hours before checking out. You have worked ${hoursWorked.toFixed(
            2
          )}h so far.`,
        });
        return;
      ...
      // directly use data
      const { data: updatedRecord } = await attendanceAPI.checkOut(payload);
        ...
        <CardContent className="flex flex-col gap-3">
          ...
          {hasCheckedIn && !hasCheckedOut && (
            <div className="text-sm">
              ...
              <span className="text-green-600">{hoursWorked.toFixed(2)}h</span>
            </div>
          )}
          <div className="flex flex-wrap gap-3">
            <Button onClick={handleCheckIn} disabled={loading || hasCheckedIn}>
              {hasCheckedIn
                ? `Checked in at ${todayRecord?.inTime}`
                : "Check In"}
            </Button>
            <Button
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {r.workedHours
                        ...
                        : calculateWorkingHoursFromTimes(r.inTime, r.outTime)}
                    </td>
                    <td className="px-4 py-3">
                      <span
